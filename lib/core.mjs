...
export function newTask(data,input,now=Date.now(),env=process.env){
 ...
 const active=data.tasks.filter(t=>['queued','running'].includes(t.status)&&now-Date.parse(t.startedAt||t.createdAt)<14*60000);if(active.length>=3){const q=active.filter(t=>t.status==='queued').length,r=active.filter(t=>t.status==='running').length;throw fail(`All 3 active slots are occupied (${r} running, ${q} queued). Queued jobs are waiting for the Netlify worker and will be retried automatically.`,429);}
...
export function newFollowupTask(data,input,now=Date.now(),env=process.env){
 ...
 parent.latestContinuationId=task.id;
 parent.steps.push({time:new Date(now).toISOString(),text:'Owner replied: '+message.slice(0,180)+(message.length>180?'…':'')});
 parent.steps.push({time:new Date(now).toISOString(),text:'Continuation started. The agent is queued to continue this conversation.'});
 return task;
}
